...
//        console.log(`<BUG (specifierRecord = ${util.inspect(specifierRecord)})>`)
    ...
    case kae.formPropertyID: // specifier.NAME or specifier.property(CODE)
        ...
        const name = appData.propertyNameForCode(seld.name) ?? `property(${aesupport.formatFourCharLiteral(seld.name)})`;
        return `${parent}.${name}`;
    ...
    if (aesupport.isNumber(want)) { // OSType
        const elementsCode = want; //.typeCodeValue(); // TO DO: check this
        var elementsName = appData.elementsNameForCode(elementsCode);
        parent += (elementsName ? `.${elementsName}` : `.elements(${aesupport.formatFourCharLiteral(elementsCode)})`);
    } else if (aesupport.isString(want)) {
        parent = `${parent}.${want}`;
    } else {
		throw new Error(`Bad 'want' (not property/element name or OSType): ${want}`);
    }
    switch (form) {
...
function applicationPathForAddressDescriptor(addressDesc) { // NSAppleEventDescriptor -> string
    ...
    if (!addressDesc || addressDesc.descriptorType() !== kae.typeKernelProcessID) { // local processes are generally targeted by PID
        throw new TypeError(
                `Unsupported address type: ${aesupport.formatFourCharLiteral(addressDesc.descriptorType())}`);
    }
...
function formatAppleEvent(appleEvent) {
    ...
    var paramsByCode = commandDef ? paramNamesByCode(commandDef) : {};
