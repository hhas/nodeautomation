...
class AEOpaqueDescriptor { // opaque object that can pack itself into an AEDescriptorBuffer
	
	constructor(type, data = null) {
		// type : UInt32 | Buffer -- a descriptorType, or a buffer containing a complete descriptor
		// data : Buffer | null -- if a descriptorType is given, the corresponding data, if any
		// note: descriptor buffer should not contain a 'dle2' header
	...
	[util.inspect.custom]() { return `[AEOpaqueDescriptor '${formatFourCharCode(this.type)}']`; }
}
...
function fileURLForLocalApplication(name) { // String -> NSURL/null
    ...
    if (name.startsWith("/")) { // absolute path to app bundle (note: full path must include .app suffix)
        ...
        var path = workspace.fullPathForApplication_(name);
...
class Keyword {
    ...
    [Symbol.toString]() {
        if (isString(this.#name)) {
            ...
            return `k.fromEnumCode(${formatFourCharCode(this.#name)})`;
        } else if (this.#type === kae.typeType) {
            return `k.fromTypeCode(${formatFourCharCode(this.#name)})`;
        } else {
    ...
    isEqual(value) {
        return value instanceof Keyword && this.#name === value.name;
    }
...
//    [Symbol.valueOf]() { return this.#path; }
    ...
    get aliasDescriptor() { // for compatibility with older apps that won't accept a typeFileURL descriptor
    	return this._descriptor.coerceToDescriptorType_(kae.typeAlias);
    }
...
module.exports = {
    ...
    // 
    isDescriptor, // is NSAppleEventDescriptor
    
    AEOpaqueDescriptor,
	isAllElementsEnum, // is `Keyword(kAEAll,typeEnumerated)`
    ...
    // comparison tests
    kAELessThanDesc:          Keyword.fromEnumCode(kae.kAELessThan),
    kAELessThanEqualsDesc:    Keyword.fromEnumCode(kae.kAELessThanEquals),
    kAEEqualsDesc:            Keyword.fromEnumCode(kae.kAEEquals),
    kAEGreaterThanDesc:       Keyword.fromEnumCode(kae.kAEGreaterThan),
    kAEGreaterThanEqualsDesc: Keyword.fromEnumCode(kae.kAEGreaterThanEquals),
    // containment tests
    kAEBeginsWithDesc:        Keyword.fromEnumCode(kae.kAEBeginsWith),
    kAEEndsWithDesc:          Keyword.fromEnumCode(kae.kAEEndsWith),
    kAEContainsDesc:          Keyword.fromEnumCode(kae.kAEContains),
    // logic tests
    kAEANDDesc:               Keyword.fromEnumCode(kae.kAEAND),
    kAEORDesc:                Keyword.fromEnumCode(kae.kAEOR),
    kAENOTDesc:               Keyword.fromEnumCode(kae.kAENOT),
};
