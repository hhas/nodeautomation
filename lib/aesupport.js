...
function isDescriptor(value) {
    try {
        return (value.getClass !== undefined // possible ObjC instance
                && value.isKindOfClass_(objc.NSAppleEventDescriptor));
    } catch(e) {
        return false;
    }
}
...
class Keyword {
    ...
    toString() {
        if (name) {
            return `k.${name}`;
        } else if (desc.descriptorType() === kae.typeEnumerated) {
    ...
    isEqual(value) {
        ...
        if (name && name === value[__keywordName]) { return true; }
        if (!(desc && value[__rawDescriptor])) { return false; }
        if (desc.descriptorType() !== value[__rawDescriptor].descriptorType()) { return false; }
        return desc.data().isEqualToData(value[__rawDescriptor].data());
    }
...
class File {
	...
	static fromHFSPath(hfsPath) {
		...
		const desc = objc.NSAppleEventDescriptor
						...
						.coerceToDescriptorType(kae.typeFileURL);
		if (!desc) {
			throw new TypeError(`Invalid path argument for File.fromHFSPath(...) constructor: ${util.inspect(hfsPath)}`);
		}
    ...
    [__packSelf](appData) { return this._desc; }
    
    // TO DO: find out which of these we need to implement
    
    toString() { return this._path; }
    valueOf() { return this._path; }
    [Symbol.toPrimitive]() { return this._path; }
    [toString]() { return this._path; }
    ...
    toHFSPath() { // -> String
        const desc = objc.NSAppleEventDescriptor.descriptorWithFileURL(objc.NSURL.fileURLWithPath_(this_path));
        return desc.coerceToDescriptorType(kae.typeUnicodeText).stringValue(); // this shouldn't fail, 
    }
