...
function formatFourCharCode(num) { // format UInt32 as four-char code (note: to get JS literal representation, use formatFourCharLiteral)
    num = Number(num);
    if (num % 1 !== 0 || num < 0 || num > UINT32_MAX) { throw new TypeError(`Not an OSType: ${num}`); }
    var result = '#'; // '#abcd' four-char code (printable ASCII) is preferred; else UInt32 '0x123456EF'
...
module.exports = {
	...
	kae, // re-export OSType constants as na.__aesupport.kae
