...
// TO DO: PS defines `RGBColor` as 'cRGv', but this isn't overriding built-in 'cRGB'; why?
...
function SDEFParser() {    
    ...
    this.attribute = function(name, element) { // String, XMLElement -> String/null
        const attr = element.attributeForName_(name);
    ...
    this.parsePropertiesOfElement = function(element) { // XMLElement // class, class-extension, record-value
        this.iter(element.elementsForName_("property"), function(element) {
            const [name, code] = this.parseKeywordElement(element);
...
//        console.log('parseDefinition: ' + element.name())
        ...
        case "class":
            ...
            const plural = element.attributeForName_("plural");
            var tmp;
            if (plural) {
        ...
        case "value-type":
            ...
            break;
        case "enumeration":
            this.iter(element.elementsForName_("enumerator"), function(element) {
        ...
        case "event":
            ...
            if (previousDef===undefined || (previousDef.eventClass === eventClass && previousDef.eventID === eventID)) {
                ...
                this.iter(element.elementsForName_("parameter"), function(element) {
                    var [name, code] = this.parseKeywordElement(element);
    ...
    this.parse = function(sdef) { // NSData
        ...
        this.iter(dictionary.elementsForName_("suite"), function(suite) {
...
function translateAttribute(node, attributeName, symbolPrefix = "") { // (XMLElement, String, String)
    const attribute = node.attributeForName_(attributeName);
    if (attribute) {
        const value = attribute.stringValue();
        if (value !== null) {
            attribute.setStringValue_(symbolPrefix + convertNameToIdentifier(value));
        }
    }
}
...
function translateType(node) { // XMLElement
    ...
    iter(node.elementsForName_("type"), function(node) { translateType(node); });
}

function setNodeAttribute(node, name, value) {
    node.addAttribute_(objc.NSXMLNode.attributeWithName_stringValue_(name, value));
}
...
function translateScriptingDefinition(data) { // NSData -> NSData
    ...
    iter(root.elementsForName_("suite"), function(suite) {
        for(var key of ["command", "event"]) {
            iter(suite.elementsForName_(key), function(command) {
                translateAttribute(command, "name");
                for (var key of ["direct-parameter", "result"]) {
                    iter(command.elementsForName_(key), function(node) {
                        translateType(node);
                    });
                }
                iter(command.elementsForName_("parameter"), function(parameter) {
                    const attribute = parameter.attributeForName_("name");
                    if (attribute) {
                        const value = attribute.stringValue();
                        if (value !== null) { attribute.setStringValue_(convertNameToIdentifier(value)); }
                    }
        ...
        for (var key of ["class", "class-extension", "record-type"]) {
            iter(suite.elementsForName_(key), function(klass) {
                translateAttribute(klass, "name");
                translateAttribute(klass, "plural");
                translateAttribute(klass, "inherits");
                iter(klass.elementsForName_("element"), function(node) {
                    translateAttribute(node, "type");
                });
                iter(klass.elementsForName_("property"), function(node) {
                    translateAttribute(node, "name");
                    translateType(node);
                    // TO DO: if type is sub-element
                });
                iter(klass.elementsForName_("contents"), function(node) {
                    translateAttribute(node, "name");
                });
                iter(klass.elementsForName_("responds-to"), function(node) {
                    translateAttribute(node, "name"); 
                    translateAttribute(node, "command");
                });
            });
        }
        iter(suite.elementsForName_("enumeration"), function(enumeration) {
            iter(enumeration.elementsForName_("enumerator"), function(enumerator) {
                translateAttribute(enumerator, "name", "k.");
            });
        });
        iter(suite.elementsForName_("value-type"), function(valueType) {
...
function exportSDEFDocumentation(applicationPath, outputPath) { // export modified SDEF for documentation use
    ...
    if (!result.writeToURL_atomically_(objc.NSURL.fileURLWithPath_(outputPath), true)) {
