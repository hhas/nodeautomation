...
function translateType(node) { // XMLElement
    ...
    iter(node('elementsForName', objc("type")), function(node) { translateType(node); });
}

function setNodeAttribute(node, name, value) {
    node('addAttribute', objc.NSXMLNode('attributeWithName', objc(name), 'stringValue', objc(value)));
}
    ...
    iter(root('elementsForName', objc("suite")), function(suite) {
        for(var key of ["command", "event"]) {
            iter(suite('elementsForName', objc(key)), function(command) {
                ...
                for (var key of ["direct-parameter", "result"]) {
                    iter(command('elementsForName', objc(key)), function(node) {
                        translateType(node);
                    });
                }
                iter(command('elementsForName', objc("parameter")), function(parameter) {
                    ...
                    if (attribute) {
                        const value = attribute('stringValue');
                        if (value !== null) { attribute('setStringValue', objc(convertNameToIdentifier(value))); }
                    }
        ...
        for (var key of ["class", "class-extension", "record-type"]) {
            iter(suite('elementsForName', objc(key)), function(klass) {
                ...
                iter(klass('elementsForName', objc("element")), function(node) {
                    translateAttribute(node, "type");
                });
                iter(klass('elementsForName', objc("property")), function(node) {
                    translateAttribute(node, "name");
                    translateType(node);
                    // TO DO: if type is sub-element
                });
                iter(klass('elementsForName', objc("contents")), function(node) {
                    translateAttribute(node, "name");
                });
        ...
        iter(suite('elementsForName', objc("value-type")), function(valueType) {
...
//exportSDEFDocumentation('/Applications/Adobe InDesign CS6/Adobe InDesign CS6.app','/Users/has/AdobeInDesignCS6.sdef');
