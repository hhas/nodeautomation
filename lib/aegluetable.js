...
function getScriptingDefinition(url) { // NSURL -> NSData
    ...
    const err = OpenScripting.OSACopyScriptingDefinitionFromURL(url.__ptr, 0, sdefPtr); // sdef arg returns CFData
    ...
    const result = objc.NSData.dataWithData_(new objc.Instance(sdef));
...
const url = objc.NSURL('fileURLWithPath', objc.ns('/Applications/TextEdit.app'));
