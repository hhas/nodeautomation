...
function exportTerminologyTable(applicationPath, outputPath) { // export raw terminology tables to JSON
    const url = objc.NSURL('fileURLWithPath', objc(applicationPath));
    const sdef = new SDEFParser();
    sdef.parse(getScriptingDefinition(url));
    const data = JSON.stringify({appleEventBridgeName: "NodeAutomation", appleEventBridgeVersion: kVersion,
...
module.exports = {
    ...
    exportTerminologyTable:  exportTerminologyTable,
};
...
exportTerminologyTable('/Applications/TextEdit.app', '/Users/has/TextEdit.node.json');
