...
// TO DO: add 'exportTerminology' function (this should export SDEFParser's terminology tables to JSON string/file; note that OSTypes would ideally be formatted as four-char/hex strings so that users can easily amend buggy terms; however, this means iterating tables on re-import to convert all those strings back to UInt32 again, so might be best to add a 'isHumanReadable' argument that outputs either compact glue)
...
function glueForApplication(url) { // NSURL -> GlueTable
...
function exportDictionary(applicationPath, outputPath) { // export modified SDEF for documentation use
...
function exportTerminologyTable(applicationPath, outputPath) {
    const url = objc.NSURL('fileURLWithPath', objc(applicationPath));
    const sdef = new SDEFParser();
    sdef.parse(getScriptingDefinition(url));
    // annoyingly, JSON doesn't use/allow hex formatted ints, so four-char codes always appear as cryptic numbers
    // TO DO: better to export as a JS module? this'd allow for comments, better FCCs, and easy absolute/relative import as module
    const data = JSON.stringify({bridgeName: "NodeAutomation", bridgeVersion: kVersion,
                                 types:sdef.types, enumerators:sdef.enumerators, properties:sdef.properties,
                                 elements:sdef.elements, commands:sdef.commands}, null, '  ');
    require('fs').writeFile(outputPath, data, 'utf8');
}
...
module.exports = {
    GlueTable:          GlueTable,
    glueForApplication: glueForApplication,
...
//exportDictionary('/Applications/TextEdit.app', '/Users/has/TextEdit.js.sdef');

//exportTerminologyTable('/Applications/TextEdit.app', '/Users/has/TextEdit.terms.js');
