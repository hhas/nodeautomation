...
// TO DO: how best to specify result/error callbacks in commands if/when async is implemented? (as 2nd and 3rd args, or as reserved keys within parametersObject?)
...
function callSubroutine(appData, parentSpecifierRecord, parametersObject) { // OBJSPEC.USERPROPERTY(…) is converted to OBJSPEC.USERCOMMAND(…)
    // note: For simplicity+sanity, this accepts positional parameters only. While four-char-code based parameters could be passed via 2nd arg (keywordParameters+attributes object), they're a brain-damaged and hopelessly underpowered mis-feature, rarely used, and never necessary. Anyone desperate enough to need them can construct and send their own subroutine call via sendAppleEvent() instead.
    var directParameter = parametersObject._;
    if (directParameter === undefined) {
        directParameter = Object.assign({_:[]}, directParameter);
    } else if (!Array.isArray(directParameter)) {
    ...
    eventParameters[objc.keyASSubroutineName] = specifier.seld;
    const commandDef = {name:specifier.seld, eventClass:objc.kAECoreSuite, eventID:objc.kASSubroutineEvent, params:{}};
    return appData.sendAppleEvent(commandDef, parentSpecifierRecord, parametersObject);
}


function doNotCall(appData, parentSpecifierRecord, parametersObject) { // OBJSPEC is non-callable
    throw new TypeError(`Object is not callable: ${aeformatter.formatSpecifier(appData, parentSpecifierRecord)}`);
}
...
function packObjectSpecifier(appData, specifierRecord) {
    if (specifierRecord.cachedDesc === null) {
        ...
        // TO DO: this won't work for untargeted specifiers, as those contain a name; simplest be for those to return a different proxy, or to modify the existing one to change its pack function (e.g. could pass alternate pack function to newSpecifier)
        //const wantType = objc.NSAppleEventDescriptor('descriptorWithTypeCode', specifierRecord.want);
        ...
        return specifierRecord.cachedDesc;
    }
}
...
function packRangeStartStop(appData, value, wantDesc) {
    if (isSpecifier(value)) { // TO DO: this won't exclude its-based specifiers but that could be checked for by asking it for its rootSpecifierDescriptor and confirming it isn't its-based [note: asking if it's app/con-based is problematic as it may also be customRoot; need to check throughout as these are legitimate—albeit ambiguous and annoying—in AS, e.g. `tell app "Finder" to get folder 1 of alias "..."`])
        return value.__nodeautomation_pack__(appData); // TO DO: check it's either app or con specifier? (TBH, there's no practical way to check if an app-based specifier is valid except to send it; dunno what happens if an its-based specifier is given, but receiving app should return errors on malformed input)
    }
    var form, seld;
...
function packLogicalTest(appData, specifierRecord) {
    if (specifierRecord.cachedDesc === null) {
        ...
        for (var item of specifierRecord.seld) {
            listDesc('insertDescriptor', item.__nodeautomation_pack__(appData), 'atIndex', 0); // TO DO: this will throw if an operand isn't a specifier (once item is packed, check it's typeCompDescriptor/typeLogicalDescriptor test too)
        }
...
function packUntargetedObjectSpecifier(targetedAppData, specifierRecord) { // targeted appData provides terminology tables
    if (specifierRecord.cachedDesc === null) {
        ...
        if (codeDesc !== null) { // is it a property name?, if it is, return `PROPERTY of PARENTSPECIFIER`
            ...
            if (codeDesc !== null) { // is it an elements name?
                want = codeDesc;
                form = specifierRecord.form;
                seld = specifierRecord.seld;
            } else {
                throw new TypeError(`Unknown property/elements name: "${name}"`);
            }
        }
        const desc = objc.NSAppleEventDescriptor('recordDescriptor')('coerceToDescriptorType', objc.typeObjectSpecifier);
...
function newRelativeSelectorMethod(selectorDataDesc) {
    return function(appData, parentSpecifierRecord) {
        return function(elementType = null) {
            if (elementType !== null && !aesupport.isKeyword(elementType)) {
                throw new TypeError(
...
function Range(start, stop, defaultWant) { // {start:OBJSPEC,stop:OBJSPEC} constructor; used in by-range specifiers
    ...
    this.__nodeautomation_pack__ = function(appData) {
        const desc = objc.NSAppleEventDescriptor('recordDescriptor')('coerceToDescriptorType', objc.typeRangeDescriptor);
...
const targetedCommandConstructors = { // allow commands to be called on targeted app-based specifiers
    "__nodeautomation_command__": function(appData, parentSpecifierRecord, name) {
        ...
        if (commandDef === undefined) { return null; } // not found
...
const targetedSpecifierConstructors = Object.assign({
    ...
    "__nodeautomation_elements__": function(appData, parentSpecifierRecord, name) {
        ...
        if (codeDesc !== null) { // is it an elements name? if it is, return `every TYPECLASS of PARENTSPECIFIER`
            return newSpecifier(appData, {
                                from:parentSpecifierRecord, 
                                want:codeDesc,
                                form:aesupport.formAbsolutePositionDesc, 
...
const targetedRootSpecifierConstructors = Object.assign({
    "sendAppleEvent": function(appData, parentSpecifierRecord) { // returns closure that accepts raw eventClass, eventID, eventParameters, eventAttributes and sends AE // TO DO: put this on targeted app object only? this would avoid having to deal with parent specifier
            return function(eventClass, eventID, parametersObject = {}, 
                            sendOptions = sendOptions.defaultOptions, timeout = defaultTimeout) {
                return appData.sendAppleEvent({eventClass:eventClass, eventID:eventID, params:{}}, 
                                              parentSpecifierRecord, parametersObject);
            }
    },
    "isRunning": function(appData, parentSpecifierRecord) {
        return appData.isRunning();
    },
    // TO DO: "launch" (this should call appData.launch, which implements special-case behavior)
    ...
    "where": function(appData, parentSpecifierRecord) { // aka 'whose'
        return function(testClause) {
            ...
            if (testAppData === undefined || testAppData.rootSpecifierDescriptor !== aesupport.kItsRootDesc) {
                const parent = aeformatter.formatSpecifier(appData, parentSpecifierRecord);
...
function newSpecifier(appData, specifierRecord) { // property/all-elements
    ...
    // TO DO: any reason not to pack specifier data immediately when AppData is available?
    return new Proxy(function() {}, { 
        apply: function(target, thisArg, argumentsList) { // this may be a convenience shortcut for SPECIFIER.get(...argumentsList), where permitted, or a SPECIFIER.$NAME(...argumentsList) subroutine call
            ...
            default:
                throw new TypeError(
                        `Too many arguments in command (expected 0 or 1 but received ${argumentsList.length}).`);
            }
        ...
        get: function(target, name) { // name may be JS slot name, app property/elements/command, 0-index (if integer)
            // is it display/instrospection? // TO DO: how best to support inspect(depth,opts)? what about toString()?
            switch (name) {
            case Symbol.toPrimitive: // TO DO: what other special JS methods do we need to handle?
            case util.inspect.custom: 
            case "toString":
                return function() { return aeformatter.formatSpecifier(appData, specifierRecord); };
            case Symbol.toStringTag:
            case "valueOf":
            case "constructor":
                return undefined;
            case "__nodeautomation_pack__":
                return function(appData) { // AppData passed to __nodeautomation_pack__() is always targeted
                    return specifierRecord.pack(appData, specifierRecord); // TO DO: use this specifier's own appData if it's targeted, and only use appData passed here if it's not?
                }
            case "__nodeautomation_appData__":
                return appData;
            case "__nodeautomation_specifierRecord__":
                return specifierRecord;
            case "__nodeautomation_inspect__": // DEBUG
                return function() { return aeformatter.inspectSpecifier(appData, specifierRecord); };
            case "customRoot":
            ...
            if (selectorFunc !== undefined) { // is it a standard selector ()
                return selectorFunc(appData, specifierRecord, name);
            }
            for (var specialFunc of [specifierRecord.selectors.__nodeautomation_property__, // TO DO: check if elements or property name should be matched first
                ...
                if (specialFunc !== undefined) {
                    ...
                    if (childSpecifier !== null) { return childSpecifier; }
                }
            }
