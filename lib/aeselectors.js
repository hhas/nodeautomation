...
function isSpecifier(value) {
    return (typeof value === 'object' || typeof value === 'function') && value.__nodeautomation_appData__ !== undefined;
}
...
function packRangeStartStop(appData, value, wantDesc) {
    ...
    case 'string':
        ...
        seld = objc.NSAppleEventDescriptor.descriptorWithString_(objc.ns(value));
        break;
...
function packLogicalTest(appData, specifierRecord) {
    ...
    for (var item of specifierRecord.seld) {
        try {
            ...
            throw new aeerrors.ParameterError(item, e.toString());
        }
    }
...
function UntargetedWantValue(name) {
    this.name = name;
    this.toString = function() { return name; }
    this.__nodeautomation_pack__ = function(targetedAppData) {
        const codeDesc = targetedAppData.elementsDescriptorByName(name);
        if (!codeDesc) { throw new Error(`Unknown elements name: "${name}"`); }
        return codeDesc;
    }
}
...
function Range(start, stop, defaultWant) { // {start:OBJSPEC,stop:OBJSPEC} constructor; used in by-range specifiers
    ...
    this.__nodeautomation_pack__ = function(appData) {
        ...
        desc.setDescriptor_forKeyword_(packRangeStartStop(appData, start, defaultWant), kae.keyAERangeStart);
        desc.setDescriptor_forKeyword_(packRangeStartStop(appData, stop, defaultWant), kae.keyAERangeStop);
        return desc;
    };
