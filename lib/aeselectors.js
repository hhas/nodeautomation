// pack/unpack and construct specifiers
...
function packUntargetedObjectSpecifier(aeBuffer, appData, untargetedSpecifierRecord) {
  ...
  let specifierRecord;
  // note: untargeted specifier records store the property/elements name string in 'want' slot; this allows it to pass through same element selector APIs as targeted specifier records; e.g.:
  // 
  // app.documents
  //  {want:"documents",form:formAbsolutePosition,seld:kAllDesc,from:kNullDesc,...,pack:packUntargetedObjectSpecifier}
  // 
  // app.documents.named("foo")
  //  {want:"documents",form:formName,seld:"foo",from:kNullDesc,...,pack:packUntargetedObjectSpecifier}
  const name = untargetedSpecifierRecord.want; 
  let code = appData.propertyCodeForName(name);
  if (code !== undefined) { // is it a property name?, if it is, return `PROPERTY of PARENTSPECIFIER`
    if (!aesupport.isAllElementsEnum(untargetedSpecifierRecord.seld)) { // both `.property` and `.elements` initially put attribute name string in 'want' property and kAllKeyword in 'seld' property
    ...
    specifierRecord = {
        from: untargetedSpecifierRecord.from, 
        want: kae.typeProperty, 
        form: kae.formPropertyID, 
        seld: new aesupport.Keyword(code),
        cachedDesc: null, 
        selectors: appData.propertySpecifierAttributes,
        call: doNotCall, // prevents e.g. `targetedApp.foo.move({to:app.bar()})`
    ...
    specifierRecord = {
        ...
        seld: untargetedSpecifierRecord.seld,
        cachedDesc: null, 
        selectors: appData.multipleElementsSpecifierAttributes,
        call: doNotCall, // ditto
        pack: packObjectSpecifier};
  }
...
function packObjectSpecifier(aeBuffer, appData, specifierRecord) {
  if (specifierRecord.cachedDesc) {
    ...
    if (specifierRecord.want === undefined) {
      throw new Error(`BUG: packObjectSpecifier received untargeted specifierRecord: ${util.inspect(specifierRecord)}`);
    }
...
function packRangeStartStop(aeBuffer, appData, value, parentWant) { // TO DO: parentWant will be OSType
  ...
  packObjectSpecifier(aeBuffer, appData, {
      want: parentWant,
      form: form,
      seld: value,
...
function packNotEqualsTest(aeBuffer, appData, specifierRecord) { // there is no kAENotEquals so pack as kAEEquals+kAENOT
  packLogicalTest(aeBuffer, appData, {
      from: {
          from: specifierRecord.from, // left operand
          form: kae.kAEEquals,        // .eq operator
          seld: specifierRecord.seld, // right operand
          cachedDesc: null, 
          selectors: logicalTestConstructors,
          call: doNotCall,
          pack: packComparisonSpecifier
      },
      form: kae.kAENOT, // .not operator
      seld: [],
      cachedDesc: null, 
      selectors: logicalTestConstructors,
      call: doNotCall,
    ...
    for (let item of specifierRecord.seld) {
      try {
        ...
        if (descType !== kae.typeCompDescriptor && descType !== kae.typeLogicalDescriptor) {
          throw new TypeError(`Not a test descriptor: ${aesupport.formatFourCharLiteral(descType)}`);
        }
      } catch (e) {
  ...
  return function(appData, parentSpecifierRecord) {
    return function(elementType = null) { // e.g. `k.word`, or null to use parent specifier's element type
      ...
      if (elementType === null) {
        ...
        throw new TypeError(`Bad previous/next argument (not a keyword): ${aeformatter.formatValue(elementType)}`);
      }
      return newSpecifier(appData, {
...
function _newManyToOneSelectorAttribute(form, selectorDataDesc) { // .first, .last, etc
  return function(appData, parentSpecifierRecord) {
    return newSpecifier(appData, {
                    ...
                    selectors: appData.singleElementSpecifierAttributes,
                    call: getDataCommand,
                    pack: parentSpecifierRecord.pack});
  };
}
...
const _specifierConstructors = { // appear on all app, con, its specifiers
  ...
  // insert at beginning/end of all elements (included here because AS is really loose about these, e.g. `tell app "TextEdit" to make new document at beginning` is a valid AE, which translates to `app('TextEdit').make({new:k.document, at:app.beginning})`)
...
const untargetedSpecifierConstructors = Object.assign({
  [__getUnknownName]: function(appData, parentSpecifierRecord, name) {
    ...
    return newSpecifier(appData, {
                    ...
                    form: kae.formAbsolutePosition, // or formProperty if name is property name
                    seld: kAllKeyword, // or Keyword(propertyCode)
  ...
  [__getZeroIndexSelector]: function(appData, parentSpecifierRecord, name) {
    if (name.match(/^-?[0-9]+$/)) { // use ELEMENTS[INT] for zero-indexed `at()` shortcut
      return newSpecifier(appData, {
                    ...
                    selectors: appData.singleElementSpecifierAttributes,
                    call: getDataCommand,
                    pack: parentSpecifierRecord.pack}); // packObjectSpecifier
    } else {
      return null; // TO DO: undefined?
    }
    ...
    apply: function(target, thisArg, argumentsList) { // this may be a convenience shortcut for SPECIFIER.get(...argumentsList), where permitted, or a SPECIFIER.$NAME(...argumentsList) subroutine call
      ...
      case 1:
        ...
        if (typeof parametersObject !== 'object') {
          throw new TypeError(`Command requires a 'parameters' object but received ${typeof parametersObject}.`);
        }
        break;
      default:
        throw new TypeError(`Command requires a single argument ('parameters' object), if any, but received ${argumentsList.length}.`);
      }
...
module.exports.targetedAppRootTables   = targetedAppRootTables; // can construct app(…)-based specifiers and also call commands on them
module.exports.untargetedAppRootTables = untargetedAppRootTables; // can construct app-based specifiers
module.exports.untargetedConRootTables = untargetedConRootTables; // can construct con-based specifiers
module.exports.untargetedItsRootTables = untargetedItsRootTables; // can construct its-based specifiers, containment/comparison tests

module.exports.doNotCall          = doNotCall; 
module.exports.getDataCommand     = getDataCommand;
module.exports.newSpecifier       = newSpecifier;
module.exports.isSpecifier        = isSpecifier;
module.exports.kAppRootDesc       = kAppRootDesc; // used in _readParameters
module.exports.Range              = Range; // used in unpack
module.exports.newRootSpecifier   = newRootSpecifier;
module.exports.packCachedDesc     = packCachedDesc; // also used in AppData to minimize unpack+repack
    
// formatter needs the following to compensate for lack of real notEqualTo and isIn operators
module.exports.packNotEqualsTest  = packNotEqualsTest;
module.exports.packIsInTest       = packIsInTest;
