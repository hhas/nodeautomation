...
var _convertedNameCache = Object.create(null); // trade memory for speed when converting keywords that appear repeatedly
    ...
    parseDefinition(element) { // XMLNode
        ...
        case "enumeration":
            for (let node of element.find("enumerator")) {
                var [name, code] = this.parseKeywordElement(node);
