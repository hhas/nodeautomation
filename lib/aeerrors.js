...
class ParameterError extends NodeAutomationError {
	...
	constructor(value, message, parentError = null) {
		...
		this.value = aeformatter.formatValue(value); // TO DO: KLUDGE: values should be stored as-is here and formatted if needed in _description, but unhandled errors in node CLI print the raw objects instead of calling util.inspect.custom, which dumps vast amounts of spew when the value in question is an object specifier (which has a complex internal structure, including huge lookup tables in AppData); formatting values here at least reduces the spew to manageable level
		this.message = message;
	}
...
class PackError extends NodeAutomationError {
	...
	constructor(value, parentError = null) {
		...
		this.value = aeformatter.formatValue(value);
	}
...
class ApplicationError extends NodeAutomationError {
	...
	constructor(replyParameters) { // application returned error; e.g. bad parameter, object not found
		...
		// TO DO: as above, these properties should hold original values, not formatted string representations, but node CLI is crap at displaying unhandled errors
		// typically returned on coercion errors (-1700)
		const expectedType = replyParameters[kae.kOSAErrorExpectedType];
		this.expectedType = expectedType === undefined ? null : aeformatter.formatValue(expectedType);
		...
		this.offendingObject = offendingObject === undefined ? null : aeformatter.formatValue(offendingObject);
	}
