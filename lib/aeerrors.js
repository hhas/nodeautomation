...
function defaultErrorMessage(code) {
    return osErrors[code] || `Error ${code}.`;
}
...
setErrorPrototype(InternalError);
...
setErrorPrototype(TerminologyError);


function ParameterError(value, errorMessage) {
    this.code = -1703;
    this.value = value;
    this.toString = function() { return `${errorMessage}: ${require('./aeformatter').formatValue(value)}`; };
}
setErrorPrototype(ParameterError);
...
setErrorPrototype(UnpackError);


function ConnectionError(code, targetID) { // TO DO: optional message parameter; e.g. see AppData.target(), which should raise this (code will appear in full error message)
    this.code = code;
    this.value = targetID;
    this.toString = function() {
        return `Can't connect to application ${require('./aeformatter').formatValue(this.value)}`;
    };
}
setErrorPrototype(ConnectionError);
...
setErrorPrototype(AppleEventManagerError);
...
function ApplicationError(appData, replyEvent) {
    ...
    this.errorMessage = function() {
        return _unpackParam(objc.keyErrorString) || defaultErrorMessage(this.code);
    };
...
setErrorPrototype(ApplicationError);
...
function CommandError(appData, commandDef, parentSpecifierRecord, parametersObject, parentError) {
    ...
    this.errorMessage = function() {
        return ((typeof parentError.errorMessage === 'function') ? parentError.errorMessage()
                                                                 : defaultErrorMessage(this.code));
    };
...
setErrorPrototype(CommandError);
