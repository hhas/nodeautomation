...
class ApplicationError extends NodeAutomationError {
	...
	constructor(appData, replyEvent) { // application returned error; e.g. bad parameter, object not found
		if (!replyEvent) {
			throw new TypeError('ApplicationError() expected replyEvent but received '+(typeof replyEvent));
		}
	...
	_unpackParam(paramKey) {
		...
		if (resultDesc) {
			try {
				...
				console.log(`BUG in ApplicationError: can't unpack ${aesupport.formatFourCharLiteral(paramKey)} param: ${e}`);
				return resultDesc;
			}
		}
		return null;
