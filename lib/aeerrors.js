...
class ApplicationError extends NodeAutomationError {
	...
	constructor(replyParameters) { // application returned error; e.g. bad parameter, object not found
		...
		super(replyParameters[kae.keyErrorNumber]);
		// additional information may be returned by an application, depending on implementation quality and error type
		this.errorMessage = replyParameters[kae.keyErrorString];
		this.expectedType = replyParameters[kae.kOSAErrorExpectedType]; // typically returned on coercion errors (-1700)
		this.offendingObject = replyParameters[kae.kOSAErrorOffendingObject]; // e.g. a command parameter that couldn't be coerced (-1700) or an object specifier that couldn't be resolved (-1728)
	}
