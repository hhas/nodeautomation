...
function _setErrorPrototype(fn) {
    ...
    fn.prototype.valueOf = fn.prototype[Symbol.toPrimitive] = fn.prototype.inspect = fn.prototype.toString = function(){
    	return `${fn.name} ${this.number}: ${this._description()}`;
    };
}
...
function InternalError(parentError) { // all known errors should be mapped to one of below; anything else is either a bug or sloppy error trapping
    this.number = -2700;
    this._description = function() { return `A bug occurred: ${parentError}`; }
}
_setErrorPrototype(InternalError);


function TerminologyError(message, number = kae.errOSACorruptTerminology) {
    this.number = number;
    this._description = function() { return `Bad terminology: ${message}`; }
}
...
function ParameterError(value, message) {
    this.number = -1703;
    this.value = value;
    this._description = function() { return `${message}: ${util.inspect(value)}`; }
}
...
function PackError(value) {
    this.number = -1700;
    this.value = value;
    this._description = function() { return `Can't pack ${typeof value}: ${value}`; }
}
...
function UnpackError(value) {
    this.number = -1700;
    this.value = value;
    this._description = function() { return `Can't unpack descriptor: ${value}`; }
}
_setErrorPrototype(UnpackError);


function ConnectionError(message, number = kae.errOSACantLaunch) {
    this.number = number;
    this._description =  function() { return defaultErrorMessage(this.number); };
}
_setErrorPrototype(ConnectionError);


function AppleEventManagerError(number) { // Apple event failed; e.g. process not found, event timed out
    this.number = number;
    this._description =  function() { return `Can't send Apple event: ${defaultErrorMessage(this.number)}`; };
}
_setErrorPrototype(AppleEventManagerError);



function ApplicationError(appData, replyEvent) { // application returned error; e.g. bad parameter, object not found
    ...
    this.number = _unpackParam(kae.keyErrorNumber);
    
    this._description = function() { return this.errorMessage() || defaultErrorMessage(this.number); };
    
    // additional error information may optionally be returned by an application, depending on implementation quality/error type
    
    this.errorMessage = function() { // application returned its own (hopefully more detailed!) description of problem
    	return _unpackParam(kae.keyErrorString);
    };
    
    this.expectedType = function() { // typically returned on coercion errors (-1700)
    	return _unpackParam(kae.kOSAErrorExpectedType);
    };
    
    this.offendingObject = function() { // typically returned when an object specifier failed to resolve (e.g. -1728)
...
function CommandError(appData, commandDef, parentSpecifierRecord, parametersObject, parentError) {
    ...
    this.number = parentError.number || 1;    
    ...
    this._description =  function() {
        try {
            var result = "";
            if (parentError.number === undefined) {
                result += `Found a bug:\n\n\t${parentError}`;
            } else {
                result +=  `${this.errorMessage() || (typeof parentError._description === 'function' ?
                    parentError._description() : defaultErrorMessage(parentError.number))}`;
            }
    ...
    this.errorMessage = function() {
        return (typeof parentError.errorMessage === 'function') ? parentError.errorMessage() : null;
    };
