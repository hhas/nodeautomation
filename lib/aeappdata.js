...
function unpackComparisonDescriptor(appData, desc) {
    const operatorDesc = desc.descriptorForKeyword_(kae.keyAECompOperator);
    ...
    if (operatorDesc && operand1Desc && operand2Desc
                                     && kComparisonOperatorCodes.includes(operatorDesc.enumCodeValue())) {
                                     
        var operand1 = appData.unpack(operand1Desc);
        var operand2 = appData.unpack(operand2Desc);
        if (operatorDesc.typeCodeValue() === kae.kAEContains && !aeselectors.isSpecifier(operand1)) { // isIn()
...
function unpackLogicalDescriptor(appData, desc) {
    const operatorDesc = desc.descriptorForKeyword_(kae.keyAELogicalOperator);
    const operandsDesc = desc.descriptorForKeyword_(kae.keyAEObject);
    if (operatorDesc && operandsDesc && operandsDesc.descriptorType() === kae.typeAEList
                                     && kLogicalOperatorCodes.includes(operatorDesc.enumCodeValue())) {
        const count = operandsDesc.numberOfItems();
        if ((operatorDesc === kae.kAENOT && count === 1) || count > 1) {
            var operands = [];
