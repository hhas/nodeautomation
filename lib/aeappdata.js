...
function processForLocalApplication(url) { // NSURL -> NSRunningApplication/null
    ...
    if (!bundle) { throw new aeerrors.ConnectionError(`Application not found: ${url}`); }
    const bundleID = bundle('bundleIdentifier');
    if (!bundleID) { throw new aeerrors.ConnectionError(`Application not found: ${url}`); }
...
function processDescriptorForLocalApplication(url, launchOptions) { // NSURL, NSWorkspaceLaunchOptions
    ...
    if (!runningProcess) {
        ...
        runningProcess = objc.NSWorkspace('sharedWorkspace')('launchApplicationAtURL', url, 
                                                             ...
                                                             'error', error.ref());
        if (!runningProcess) { throw new TypeError(`Can't launch ${url}: ${error}`); }
    }
...
function launchApplicationAtURL(url) {
    ...
    const runningProcess = objc.NSWorkspace('sharedWorkspace')('launchApplicationAtURL', url, 
                                                               ...
                                                               'error', error.ref());
    if (!runningProcess) { throw new aeerrors.ConnectionError(error('code'), url); }
}
...
function AppData(targetType, targetID, options) {
    ...
    this.target = function() {  // TO DO: should throw aeerrors.ConnectionError(), not TypeError
        if (!this._targetDescriptor) {
            ...
            case "named":
                ...
                if (!url) { throw new TypeError(`Application not found: ${this.targetID}`); }
