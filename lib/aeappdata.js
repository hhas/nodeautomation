...
// TO DO: decide policy on unpacking NSString, NSDate and other bridged values; return wrapped ObjC instance (quicker), or cast to native JS types (safer with user code that uses typeof/instanceof to check returned values' types), or make it a toggleable compatibility option (that is safe by default)?

// TO DO: if implementing alwaysFullyUpackSpecifiers flag on AppData, assign the original form-specific pack func to specifierRecord.pack and null to specifierRecord.cachedDesc to force a full re-pack from root (or maybe just call original constructor funcs)

// TO DO: worth putting callback hooks on AppData for packing unknown JS values, unpacking unknown AEDescs, packing unknown AE params/attrs, etc? (might be best way to do compatibility hooks in general; saves having to implement explicity flags for rare corner cases - e.g. old file object types, fully unpacking+repacking specifiers, etc); alternatively, users may monkey-patch existing pack/unpack methods to handle those types first before delegating to original
...
// TO DO: where should error constructors go? (make sure they're exported too)

// TO DO: implement these plus CommandError and any others we need (check JS docs on implementing custom error objects)

function PackError(value) { // TO DO: take message and/or sub-error as optional args
    this.errorNumber = -1700;
    this.value = value;
    this.toString = function() { return `(${this.errorNumber}) Can't pack value: ${this.value}` }
}

function UnpackError(desc) { // TO DO: take message and/or sub-error as optional args
    this.errorNumber = -1700;
    this.value = desc;
    this.toString = function() { return `(${this.errorNumber}) Can't unpack descriptor: ${this.value}` }
}

function ConnectionError(errorNumber, targetID) { // TO DO: take message and/or sub-error as optional args
    this.errorNumber = errorNumber;
    this.value = targetID;
    this.toString = function() { return `(${this.errorNumber}) Can't connect to application: ${this.value}` }
}

function CommandError(errorNumber) {
    this.errorNumber = errorNumber;
}


/****************************************************************************************/
// -[NSWorkspace launchApplicationAtURL:options:configuration:error:], plus relaunch options
...
const kLaunchOptions = {
    ...
    launchDefault:                  0x00010000, // launchAsync
};

const kRelaunchOptions = {
    neverRelaunch:   1,
    limitedRelaunch: 2,
    alwaysRelaunch:  3,
    defaultRelaunch: 2,
...
function readLaunchOptions(options) {
    var launch = 0, relaunch = 0;
    if (!(typeof options === 'object' && options instanceof Array)) { options = [options]; }
    for (var option in options) {
        if (kLaunchOptions[option] !== undefined) {
            launch |= kLaunchOptions[option];
        } else if (kRelaunchOptions[option] !== undefined) {
            relaunch = kRelaunchOptions[option];
        } else {
            throw new TypeError(`Bad launch options: ${options}`);
        }
    }
    return [launch === 0 ? kLaunchOptions.launchDefault : launch,
            relaunch === 0 ? kRelaunchOptions.defaultRelaunch : relaunch];
}
...
// TO DO: may be better as object, as reply options are mutually exclusive; ditto interact options; in addition we need some way to supply terminology modules

// TO DO: add a k.doNotUnpackReply option for returning reply descriptor as-is
...
function readSendOptions(options) {
    var sendFlags = 0;
    if (!(typeof value === 'object' && value instanceof Array)) { value = [value]; }
    for (item of value) {
        if (typeof item !== 'object') {
            throw new TypeError(`Bad "sendOptions" attribute: ${aeformatter.formatValue(value)}`);
        }
...
function readIgnoringOptions(value) {
    ...
    for (var item of value) {
        if (typeof item !== 'object') {
            throw new TypeError(`Bad "ignoring" attribute: ${aeformatter.formatValue(value)}`);
        }
        ...
        considerIgnoreFlags &= ~optionDef.consider; // TO DO: check JS bitwise ops are always 32-bit
...
// kAEDefaultTimeout = -1
// kNoTimeOut = -2
const kDefaultTimeout = 120 // bug workaround: NSAppleEventDescriptor.sendEvent(options:timeout:) method's support for kAEDefaultTimeout=-1 and kNoTimeOut=-2 flags is buggy <rdar://21477694>, so for now the default timeout is hardcoded here as 120sec (same as in AS)
...
function localRunningApplication(url) { // NSURL -> NSRunningApplication/null
    const bundle = objc.NSBundle('bundleWithURL', url);
    if (bundle === null) { throw new ConnectionError(`Application not found: ${url}`); }
    const bundleID = bundle('bundleIdentifier');
    if (bundleID === null) { throw new ConnectionError(`Application not found: ${url}`); }
    ...
    if (foundProcesses.length === 1) {
        ...
        for (process of foundProcesses) {
            if (process('bundleURL')('isEqual', url)) { // TO DO: confirm this checks for FS identity, not path string equality; if not, use NSURLFileResourceIdentifierKey
                return process;
            }
        }
    }
...
function fileURLForLocalApplication(name) { // String -> NSURL/null
    if (name[0] === "/") { // full path (note: path must include .app suffix)
        ...
        if (path === null) { path = workspace('fullPathForApplication', `${name}.app`); }
        return path === null ? null : objc.NSURL('fileURLWithPath', path);
    }
}


function processDescriptorForLocalApplication(url, launchOptions) { // NSURL, NSWorkspaceLaunchOptions
    ...
    var runningProcess = localRunningApplication(url);
    if (runningProcess == null) {
        ...
        runningProcess = objc.NSWorkspace('sharedWorkspace')('launchApplicationAtURL', url, 
                                                             ...
                                                             'error', error.ref());
        if (runningProcess === null) { throw new TypeError(`Can't launch ${url}: ${error}`); }
    }
...
function launchApplicationAtURL(url) {
    ...
    const runningProcess = objc.NSWorkspace('sharedWorkspace')('launchApplicationAtURL', url, 
                                                               ...
                                                               'error', error.ref());
    if (runningProcess === null) { throw new ConnectionError(error('code'), url); }
}
...
// used by unpacked specifiers to defer full unpacking until needed (when formatting for display)

// TO DO: would be better just unwinding typeObjectSpecifier descs directly to specifier records
function unpackLazyObjectSpecifier(appData, desc, fullyUnpack) {
    function unpackFunc(appData) {
        const result = appData.unpack(desc, true);
        if (typeof result === 'object' && result.__nodeautomation_specifierRecord__ !== null) {
            return result.__nodeautomation_specifierRecord__;
        } else {
            return result;
        }
    }
    return fullyUnpack ? unpackFunc(appData) : {__nodeautomation_unpackFunc__:unpackFunc};
}
...
function unpackComparisonDescriptorRecord(appData, desc) {
    ...
    if (operatorType !== null && operator1Desc !== null && operator2Desc !== null 
...
function unpackLogicalDescriptor(appData, desc) {
    ...
    if (operatorType !== null && operandsDesc !== null && operandsDesc('descriptorType') === objc.typeAEList
...
function unpackSpecifierRecord(appData, desc, fullyUnpack) {
    ...
    case objc.typeObjectSpecifier:
        ...
        case objc.formProperty:
        case objc.formUserProperty:
        ...
        case objc.formRange:
            ...
            break;
        case objc.formTest:
            seld = unpackTestDescriptor(appData, seld); // TO DO: this will probably return specifierRecord, which will need wrapped as specifier
            selectors = appData.multipleElementsSpecifierAttributes;
            break;
        default:
            throw new TypeError(`Malformed object specifier (unknown form): ${desc}`);
        }
        return {from:unpackSpecifierRecord(appData, desc('descriptorForKeyword', objc.keyAEContainer), fullyUnpack),
                want:want,
                form:form,
                seld:seld,
                cachedDesc:desc,
...
function AppData(targetType, targetID, launchOptions) {
    [this.workspaceLaunchOptions, this.relaunchMode] = readLaunchOptions(launchOptions);
    // used by formatter
    ...
    this.target = function() {
        if (this._targetDescriptor === null) {
            var desc;
            switch (this.targetType) {
            case "named":
                if (typeof this.targetID !== 'string') {
                    throw new TypeError(`app.named(...) requires a name or path string but received: ${this.targetID}`);
                }
                var url = fileURLForLocalApplication(this.targetID);
                if (url === null) { throw new TypeError(`Application not found: ${this.targetID}`); }
                desc = processDescriptorForLocalApplication(url, this.workspaceLaunchOptions);
                break;
            case "at": // eppc: URL
                if (typeof this.targetID !== 'string') {
                    throw new TypeError(`app.at(...) requires an "eppc:" URL string but received: ${this.targetID}`);
                }
                var url = objc.NSURL('URLWithString', objc(this.targetID));
                if (url === null || url('scheme').toString().toLowerCase() !== 'eppc') {
                    throw new TypeError(`app.at(...) requires an "eppc:" URL string but received: ${this.targetID}`);
                }
            ...
            case "ID":
                ...
                case 'string': // bundleIdentifier
                    ...
                    var runningProcess = objc.NSWorkspace('sharedWorkspace')(
                                                        ...
                                                        'error', error.ref());
                    if (runningProcess === null) {
                        throw new TypeError(`Can't launch application "${this.targetID}": ${error}`);
                    }
    ...
    this.isRunning = function() {
        ...
        case 'named': // application's name (.app suffix is optional) or full path
            var url = fileURLForLocalApplication(this.targetID);
            return url !== null && localRunningApplication(url) !== null;
        case 'at': // "eppc" URL
        ...
        case 'ID':
            ...
            case 'number': // PID
                return objc.NSRunningApplication('runningApplicationWithProcessIdentifier', this.targetID) !== null;
    ...
    this._sendAppleEvent = function(event, sendOptions, timeoutInSeconds) { // used by sendAppleEvent()
        // returns [null, NSError] on AEM errors only, e.g. 'process not found'
        // application results and errors (if any) are always reported via replyEvent
        var error = objc.alloc(objc.NSError, objc.NIL);
        var replyEvent = event('sendEventWithOptions', sendOptions, 'timeout', timeoutInSeconds, 'error', error.ref());
        return [replyEvent, error];
    };
    
    //
    
    this._sendLaunchEvent = function(processDescriptor) { // returns AEM/reply event error code
        ...
        if (replyEvent === null) { return error('code'); }
        ...
        return (errorDesc !== null && errorDesc('int32Value') !== -1708) ? errorDesc('int32Value') : 0;
    }
    ...
    this.launch = function() { // called by Application.launch()
        ...
        if (this.isRunning()) {
            ...
            if (errorNumber !== 0) { throw new ConnectionError(errorNumber, this.targetID); }
        } else {
            switch (this.targetType) {
            case 'named':
                var url = fileURLForLocalApplication(this.targetID);
            ...
            throw new ConnectionError(10000, this.targetID); // TO DO: what error number to use here?
        }
    }
    ...
    this.sendAppleEvent = function(commandDef, parentSpecifierRecord, parametersObject) {
        // note: 'commandDef' may or may not contain 'name', depending who sent it
//        console.log('SEND', [commandDef, aeformatter.formatSpecifier(this, parentSpecifierRecord), parametersObject]);
        var replyEvent = null, nsError = null;
//        try {
            if (typeof parametersObject !== 'object') {
                throw new TypeError(`Bad command argument: not a parameters object.`);
            }
            ...
            const appleEvent = objc.NSAppleEventDescriptor('appleEventWithEventClass', commandDef.eventClass, 
            ...
            for (key in parametersObject) {
                try {
                    ...
                    if (value === undefined) { throw new TypeError("Parameter cannot be undefined."); }
                    ...
                    if (paramCode !== undefined) {
                        ...
                        case "asType": // must be keyword
                            if (typeof value !== 'object' || value.__nodeautomation_keywordName__ === undefined) {
                                throw new TypeError(`Bad "asType" attribute: ${aeformatter.formatValue(value)}`);
                            }
                        ...
                        case "sendOptions": // all send flags; [array of] keywords, e.g. [k.ignoreReply,...]
                            ...
                            break;
                        case "withTimeout": 
                            // note: -[NSAppleEventDescriptor sendEventWithOptions:timeout:] is buggy (it multiplies timeout in seconds by 60 to get timeout in ticks, but should only do this for +ve values, not for -ve values which are flags); therefore we use our own 'default timeout' (null) and 'no timeout' (0) values
                            if (value !== null) { // 'kAEDefaultTimeout = -1', but we use 'kDefaultTimeout = 120' above
                                try {
                                    timeoutInSeconds = aesupport.SInt32(value);
                                    if (timeoutInSeconds <= 0) { // 'kNoTimeOut = -2' but we use <=0 as 'no timeout'
                                        timeoutInSeconds = -2; // fortunately AEM treats any value <= -2 as 'no timeout'
                                    }
                                } catch(e) {
                                    throw new TypeError("Timeout attribute must be a non-negative integer or null.");
                                }
                            }
                            break;
                        ...
                        default: // if four-char code (e.g. '#docu', '0x646f6375') pack as param, else throw 'unknown'
                            ...
                            try {
                                ...
                                throw new TypeError(`Unknown command parameter/attribute: "${key}"`);
                            }
                            appleEvent('setParamDescriptor', this.pack(value), 'forKeyword', rawParamCode);
                        }
                    }
                } catch(e) {
                    throw new TypeError(`Bad "${key}" value: ${value}. ${e}`);
                }
            }
            ...
            if (parentSpecifierRecord.form !== aesupport.kSpecifierRoot) {
                ...
                if (commandDef.eventClass === objc.kAECoreSuite && commandDef.eventID === objc.kAECreateElement) {
                    ...
                    if (appleEvent('paramDescriptorForKeyword', objc.keyAEInsertHere) === null) {
            ...
            // TO DO: if sendOptions is supplied but does not specify xxxxReply/xxxxInteract options, add defaults for these here
            
            [replyEvent, nsError] = this._sendAppleEvent(appleEvent, sendOptions, timeoutInSeconds);
            
            if (replyEvent === null) { // handle errors raised by Apple Event Manager (e.g. timeout, process not found)
                if (kRelaunchableErrorCodes.includes(nsError('code')) && this.isRelaunchable() 
                        && (this.relaunchMode === kRelaunchOptions.alwaysRelaunch
                               || (this.relaunchMode === kRelaunchOptions.limitedRelaunch 
                    ...
                    this._targetDescriptor = null;
                    const appleEvent2 = objc.NSAppleEventDescriptor('appleEventWithEventClass', commandDef.eventClass, 
                                                                    'eventID', commandDef.eventID, 
                                                                       'targetDescriptor', this.target(), 
                                                                       'returnID', objc.kAutoGenerateReturnID, 
                                                                       'transactionID', objc.kAnyTransactionID);
                    for (var i = 1; i <= event.numberOfItems; i++) {
                        appleEvent2('setParamDescriptor', appleEvent('descriptorAtIndex', i), 
                                    'forKeyword', appleEvent('keywordForDescriptorAtIndex', i));
                    }
                    for (var key of [objc.keySubjectAttr, objc.enumConsiderations, objc.enumConsidsAndIgnores]) {
                        appleEvent2('setAttributeDescriptor', appleEvent('attributeDescriptorForKeyword', key),
                                    'forKeyword', key);
                    }
                    [replyEvent, nsError] = this._sendAppleEvent(appleEvent2, sendOptions, timeoutInSeconds);
                }
                if (replyEvent === null) {
                    throw new TypeError(`AEM error: ${nsError}`); // TO DO: skip details if we just trap and rethrow below?
                }
            }
//            console.log('SENT EVENT: ', appleEvent, '\nREPLY EVENT:', replyEvent, '\nAEM ERROR:  ', nsError); // DEBUG
            if (sendOptions & kSendOptions.waitReply) {
// TO DO: add k.doNotUnpackReply send option
...
//              } 
                ...
                if (errorNumberDesc !== null && errorNumberDesc('int32Value') !== 0) { // application error occurred
                    throw new TypeError(`Application error: ${ // TO DO: skip details if we just trap and rethrow below?
                                        replyEvent('paramDescriptorForKeyword', objc.keyErrorNumber)('int32Value')}`);
                } else {
                    const resultDesc = replyEvent('paramDescriptorForKeyword', objc.keyDirectObject);
                    if (resultDesc !== null) { return this.unpack(resultDesc); }
                } // no return value or error, so fall through

            } else if (sendOptions & kSendOptions.queueReply) { // get the return ID that will be used by the reply event so that client code's main loop can identify that reply event in its own event queue later on // TO DO: if caller passes async closures, how to invoke those? (is this possible? practical? will need to see how node.js/NodObjC does event loops; it may be we have to defer this to users)
                const returnIDDesc = event('attributeDescriptorForKeyword', objc.keyReturnIDAttr);
                if (returnIDDesc === null) { throw new TypeError("Can't get keyReturnIDAttr."); } // sanity check
                return this.unpack(returnIDDesc);
            }
            return null; // no result
//        } catch(e) { // TO DO: trap and rethrow basic Pack/Unpack/AEM/Application errors with full details (appleEvent, replyEvent, nsError) attached
//
//            throw new TypeError(`Command error: ${e}`);
//        }
    };     // TO DO: what about async? (that needs access to main event loop to extract reply events as they arrive; no idea how that'll work yet, so leave for later)
    ...
    this.pack = function(value) {
        ...
        case "number": // TO DO: NaN, Infinity, etc?
            return objc.NSAppleEventDescriptor( aesupport.isSInt32(value) ? 
                    'descriptorWithInt32' : 'descriptorWithDouble', value);
        case "string":
        ...
        if (value.__nodeautomation_pack__ !== undefined) { // value is self-packing, given a targeted AppData instance
            ...
            var customType = value.class, isCustomRecordType = false;
            if (customType === undefined) { customType = value[objc.pClass]; } // TO DO: need to check how numeric keys actually look up; may need to identify class slot by iterating them all
            var recordDesc = objc.NSAppleEventDescriptor('recordDescriptor');
            if (customType !== undefined && aesupport.isKeyword(customType)) {
            ...
            for (var key in value) {
                ...
                if (keyCode === undefined) {
                    if (key[0] === '$') {
                        if (userFieldsDesc === null) { userFieldsDesc = objc.NSAppleEventDescriptor('listDescriptor'); }
                ...
                recordDesc('setDescriptor', valueDesc, 'forKeyword', keyCode);
                    ...
                    throw new TypeError(`Can't pack "${key}" field: ${e}`);
                }
            }
            if (userFieldsDesc !== null) {
                recordDesc('setDescriptor', userFieldsDesc, 'forKeyword', objc.keyASUserRecordFields);
            }
            return recordDesc;
        } else if (typeof value === 'function' && value.getClass !== undefined) { // possible ObjC instance // TO DO: check NodObjC API for dedicated type checking functions
            if (aesupport.isDescriptor(value)) { return value; }
            //TO DO: bridge other standard objc classes: NSArray, NSString, etc?
        }
        throw new PackError(value);
    };
    ...
    this.unpack = function(desc, fullyUnpack = false) {
        ...
        case objc.typeVersion:
            ...
            if (nsString === null) { throw new TypeError(`Can't unpack malformed string descriptor: ${desc}`); }
            return nsString.toString();
        case objc.typeSInt32:
        case objc.typeSInt16:
        case objc.typeUInt16:
            return desc('int32Value'); // TO DO: given JS's native representation is always double, is there any point unpacking as int?
        ...
        case objc.type128BitFloatingPoint: // caution: lossy precision
            ...
            if (doubleDesc === null) { throw new TypeError(`Can't coerce 128-bit float to double: ${desc}`); }
        ...
        case objc.typeLongDateTime:
            ...
            if (nsDate === null) { throw new TypeError(`Can't unpack malformed date descriptor: ${desc}`); }
            return new Date(nsDate);
        case objc.typeObjectSpecifier:
            // TO DO: implement alwaysFullyUnpackSpecifiers compatibility flag on appData; if true always fully unpack and omit cachedDesc, emulating exact AS behavior (iView Media Pro is only known app that requires this due to combination of two subtle design flaws in its AEOM implementation that prevent it roundtripping its own AEDescs correctly)
            return aeselectors.newSpecifier(this, unpackSpecifierRecord(this, desc, fullyUnpack));
        case objc.typeInsertionLoc:
            return aeselectors.newSpecifier(this, {
                    from:unpackLazyObjectSpecifier(this, desc('descriptorForKeyword', objc.keyAEObject), fullyUnpack),
        ...
        case objc.typeKeyword:
            ...
            return aesupport.Keyword(name === undefined ? code : name);
        case objc.typeEnum:
            var code = desc('enumCodeValue');
            var name = this.typeNameByCode(code);
            return aesupport.Keyword(name === undefined ? code : name);
        // file types
        case objc.typeAlias:
        ...
        case objc.typeBookmarkData: // may be problematic/buggy
            return new File(desc('fileURLValue')('path').toString()); // TO DO: what about caching/roundtripping original descriptor? (there are pros and cons to this, e.g. potentially better compatibility with quirky old Carbon apps vs opaque differences in behavior between alis/bmrk and furl/fsrf. TBH roundtripping compatibility is probably less of a concern now than it was a decade ago, so for now probably best to go with 'dumb' path-only File objects that always use typeFileURL descriptors)
        case objc.typeNull:
        ...
        // TO DO: Buffer doesn't provide methods for reading with native endianness, so for now hardcode as LE (i386/x86_64)
    ...
    this.unpackAERecord = function(desc) {
        ...
        if (customTypeCode !== objc.typeAERecord) {
            ...
            result.class = new aesupport.Keyword(name === undefined ? customTypeCode : name);
        }
        for (var i = 1; i <= desc('numberOfItems'); i++) {
            ...
            if (key === objc.keyASUserRecordFields) {
                if (valueDesc('descriptorType') !== objc.typeAEList || valueDesc('numberOfItems') % 2 !== 0) {
                    throw new TypeError(`Malformed keyASUserRecordFields value in record: ${valueDesc}`);
                }
                ...
                result[name === undefined ? aesupport.formatFourCharCode(key) : name] = this.unpack(valueDesc);
            }
        }
        return result;
    }
    ...
    // caution: these tables are lazily initialized, so always access via functions below, never directly
    
    this._typesByName = {};
    this._typesByCode = {};
    this._propertiesByName = {};
    this._propertiesByCode = {};
    this._elementsByName = {};
    this._elementsByCode = {};
    this._commandsByName = {};
    
    // stub methods load terminology and replace themselves on first use
    ...
    this.typeNameByCode = function(code) {
        return _loadTerminology(this).typeNameByCode(code);
    };
    
    // pack/unpack specifiers
    
    // TO DO: might consider caching previously created AEDescs
...
const _terminologyAccessors = { // these will be bound to AppData the first time terminology lookup is performed

    typeDescriptorByName: function(name) { // used to pack keyword objects
        const typeDef = this._typesByName[name]; // {name:[descriptorConstructorName,code],...}
        if (typeDef === undefined) { throw new TypeError(`Unknown type/enum/property name: "${name}"`); }
        return objc.NSAppleEventDescriptor(typeDef[0], typeDef[1]);
    },
    
    typeCodeByName: function(name) { // used to look up property keys
        const typeDef = this._typesByName[name];
        return (code === undefined) ? null : typeDef[1];
    },
    
    typeNameByCode: function(code) {
    ...
    // TO DO: might consider caching previously created AEDescs
    propertyDescriptorByName: function(name) {
        const code = this._propertiesByName[name];
        if (code === undefined) { return null; }
        return objc.NSAppleEventDescriptor('descriptorWithTypeCode', code);
    },
    
    propertyNameByCode: function(code) {
        return this._propertiesByCode[code];
    },
    
    elementsDescriptorByName: function(name) {
        const code = this._elementsByName[name];
        if (code === undefined) { return null; }
        return objc.NSAppleEventDescriptor('descriptorWithTypeCode', code);
    },
    
    elementsNameByCode: function(code) {
        return this._elementsByCode[code];
    },
    
    // pack/unpack Apple event
    
    commandDefinitionByName: function(name) {
        return this._commandsByName[name];
    },
    
    commandDefinitionByCode: function(eventClass, eventID) {
        throw new TypeError('TO DO'); // TO DO: not currently implemented as it's only needed for translation tools
    },
};
...
function _loadTerminology(appData) { // TO DO: need to merge this into target as it may launch process, in which case we don't want to launch it a second time if 'new instance' flag is set
    ...
    case "named":
        url = fileURLForLocalApplication(appData.targetID);
        if (url === null) { throw new TypeError(`Application not found: ${appData.targetID}`); }
        break;
    case "at": // eppc: URL
    ...
    case "ID":
        ...
        case 'string': // bundleIdentifier
            ...
            if (appPath === null) { throw new TypeError(`Can't find ${appData.targetID}: ${error}`); }
            url = objc.NSURL('fileURLWithPath', appPath);
            break;
        case 'number': // ProcessID
            try {
                var runningProcess = objc.NSRunningApplication('runningApplicationWithProcessIdentifier', 
                                                                aesupport.SInt32(appData.targetID));
            } catch(e) {
                throw new TypeError(`app.ID(...) received bad process ID number: ${appData.targetID}`);
            }
            if (runningProcess === null) { throw new TypeError(`Can't find running application with PID ${appData.targetID}: ${error}`);
            }
            url = runningProcess('bundleURL');
            break;
        case 'function': // NSAppleEventDescriptor?
            if (aesupport.isDescriptor(appData.targetID)) {
                // TO DO: problem here is that we can send ascr/gsdf event, but macOS bugs prevent valid SDEF data being returned if app bundle doesn't have an .sdef file (i.e. it should automatically transcode AETE/scriptTerminology if that's what app contains, but it doesn't)
                throw new TypeError(`TO DO: can't yet get terminology for address descriptor ${appData.targetID}`);
    ...
    const glueTable = new aegluetable.GlueTable();
    glueTable.addSDEFAtURL(url);
...
function UntargetedAppData(selectortables) {
    ...
    this.targetID = null; // TO DO: check; shouldn't be needed; delete
    
    // TO DO: these shouldn't be needed (untargeted specifiers should know not to call them, so they're really just checking for bugs); delete
    this.propertyDescriptorByName = function(name) { throw new TypeError('Not supported.'); }
    this.elementsDescriptorByName = function(name) { throw new TypeError('Not supported.'); }
    this.commandDefinitionByName = function(name) { throw new TypeError('Not supported.'); }
    this.propertyNameByCode = function(code) { throw new TypeError('Not supported.'); }
    this.elementsNameByCode = function(code) { throw new TypeError('Not supported.'); }
    this.commandDefinitionByCode = function(code) { throw new TypeError('Not supported.'); }
    this.sendAppleEvent = function() { throw new TypeError('Not supported.'); }
...
module.exports = {
    ...
    // TO DO: export errors from here? or move them to aesupport?
};
