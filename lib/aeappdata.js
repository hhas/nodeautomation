...
function fileURLForLocalApplication(name) { // String -> NSURL/null
    ...
    if (name.startsWith("/")) { // absolute path to app bundle (note: full path must include .app suffix)
        ...
        if (path === null && !path.toLowerCase().endsWith('.app')) {
...
function AppData(targetType, targetID, options) {
    ...
    this.sendAppleEvent = function(commandDef, parentSpecifierRecord, parametersObject) {
        ...
        try {
            ...
            // unpack application error/result, if any
    ...
    this.unpack = function(desc, fullyUnpack = false) {
        ...
        case objc.typeEnum:
            ...
            return name !== undefined ? new aesupport.Keyword(name) : new aesupport.Keyword(null, desc);
        // file types
        case objc.typeAlias:
...
const _terminologyAccessors = { // these will be bound to AppData the first time terminology lookup is performed, replacing stub terminology lookup methods
    ...
    typeDescriptorByName: function(name) { // used to pack keyword objects; throws 'unknown' error if not found
        var desc = this._typeDescriptorsByName[name];
        if (!desc) {
            const typeDef = this._typesByName[name]; // {name:[descriptorConstructorName,code],...}
            if (typeDef === undefined) { throw new TypeError(`Unknown type/enum/property name: "${name}"`); } // TO DO: error type?
            desc = this._typeDescriptorsByName[name] = objc.NSAppleEventDescriptor(typeDef[0], typeDef[1]);
        }
        return desc;
    },
    
    typeCodeByName: function(name) { // used to look up property keys; returns null if not found
        const typeDef = this._typesByName[name];
        return (code === undefined) ? null : typeDef[1];
    },
    
    typeNameByCode: function(code) { // used to unpack keyword objects; returns `undefined` if not found
        return this._typesByCode[code];
    },
    
    // pack/unpack specifiers
    
    propertyDescriptorByName: function(name) { // used to pack object specifiers; returns null if not found
        var desc = this._propertyDescriptorsByName[name];
        if (!desc) {
            if (desc === null) { return null; }
            const code = this._propertiesByName[name];
            if (code === undefined) { this._propertyDescriptorsByName[name] = null; return null; }
            desc = this._propertyDescriptorsByName[name] = objc.NSAppleEventDescriptor('descriptorWithTypeCode', code);
        }
        return desc;
    },
    
    propertyNameByCode: function(code) {  // TO DO: what if it's an ambiguous term that's used as both property and elements name, in which case it needs disambiguated
        return this._propertiesByCode[code];
    },
    
    elementsDescriptorByName: function(name) { // used to pack object specifiers; returns null if not found
        var desc = this._elementsDescriptorsByName[name];
        if (!desc) {
            if (desc === null) { return null; }
            const code = this._elementsByName[name];
            if (code === undefined) { this._elementsDescriptorsByName[name] = null; return null; }
            desc = this._elementsDescriptorsByName[name] = objc.NSAppleEventDescriptor('descriptorWithTypeCode', code);
        }
        return desc;
    },
    
    elementsNameByCode: function(code) { // TO DO: as above
        return this._elementsByCode[code];
    },
    
    // pack/unpack Apple event
    
    commandDefinitionByName: function(name) { // used to construct AppleEvent descriptor, returns undefined if not found 
        ...
        case "ID":
            ...
            case 'function': // NSAppleEventDescriptor?
                if (aesupport.isDescriptor(appData.targetID)) {
                    ...
                    throw new TypeError(`Can't automatically retrive terminology when targeting app by AEAddressDescriptor ${appData.targetID}; supply a static terminology object instead.`);
