...
const kLaunchOptions = {
    launchAndPrint:                 objc.NSWorkspaceLaunchAndPrint,
    ...
    launchAsync:                    objc.NSWorkspaceLaunchAsync,
    ...
    launchDefault:                  objc.NSWorkspaceLaunchDefault, // = launchAsync
};
...
kRelaunchOptions.defaultRelaunch = kRelaunchOptions.limitedRelaunch;


// TO DO: fix this; options is object containing (amongst other things) terminology options
...
function localRunningApplication(url) { // NSURL -> NSRunningApplication/null
    ...
    if (bundle === null) { throw new aeerrors.ConnectionError(`Application not found: ${url}`); }
    const bundleID = bundle('bundleIdentifier');
    if (bundleID === null) { throw new aeerrors.ConnectionError(`Application not found: ${url}`); }
...
function launchApplicationAtURL(url) {
    ...
    const runningProcess = objc.NSWorkspace('sharedWorkspace')('launchApplicationAtURL', url, 
                                                               ...
                                                               'error', error.ref());
    if (runningProcess === null) { throw new aeerrors.ConnectionError(error('code'), url); }
}
...
function AppData(targetType, targetID, launchOptions) {
    ...
    this.launch = function() { // called by Application.launch()
        ...
        if (this.isRunning()) {
            ...
            if (errorNumber !== 0) { throw new aeerrors.ConnectionError(errorNumber, this.targetID); }
        } else {
            switch (this.targetType) {
            case 'named':
                var url = fileURLForLocalApplication(this.targetID);
            ...
            throw new aeerrors.ConnectionError(10000, this.targetID); // TO DO: what error number to use here?
        }
    }
    ...
    this.sendAppleEvent = function(commandDef, parentSpecifierRecord, parametersObject) {
        ...
        try {
                    ...
                    if (paramCode !== undefined) {
                        ...
                        case "asType": // must be keyword
                            if (typeof value !== 'object' || value.__nodeautomation_keywordName__ === undefined) {
                                throw new ParameterError(value, 'Bad asType attribute (not a keyword)');
                            }
                        ...
                        case "withTimeout": 
                            ...
                            if (value !== null) { // 'kAEDefaultTimeout = -1', but we use 'kDefaultTimeout = 120' above
                                try {
                                    ...
                                    throw new ParameterError(value, 
                                            "Bad timeout attribute (not a non-negative integer or null)");
                                }
                            }
                            break;
                        ...
                        default: // if four-char code (e.g. '#docu', '0x646f6375') pack as param, else throw 'unknown'
                            ...
                            try {
                                ...
                                throw new ParameterError(value, `Unknown parameter/attribute: "${key}"`);
                            }
                            appleEvent('setParamDescriptor', this.pack(value), 'forKeyword', rawParamCode);
                        }
                    }
                } catch (e) {
                    if (!(e instanceof ParameterError)) { // PackError/bugs
                        if (e instanceof PackError) { e = `can't pack ${typeof value} as descriptor`; }
                        e = new ParameterError(value, `Bad "${key}" parameter (${e})`);
                    }
                    throw e;
                }
            }
            ...
            if (replyEvent === null) { // handle errors raised by Apple Event Manager (e.g. timeout, process not found)
                ...
                if (replyEvent === null) { throw new aeerrors.AppleEventManagerError(nsError('code')); }
            }
...
//              } 
                ...
                if (returnIDDesc === null) { // sanity check
                    throw new ParameterError(null, "Can't get keyReturnIDAttr from reply event");
                }
        ...
        if (value.__nodeautomation_pack__ !== undefined) { // value is self-packing, given a targeted AppData instance
            ...
            for (var key in value) {
                try {
                    ...
                    if (keyCode === undefined) {
                        if (key[0] === '$') {
                            if (userFieldsDesc === null) {
                                userFieldsDesc = objc.NSAppleEventDescriptor('listDescriptor');
                            }
                    ...
                    throw new PackError(value, `Can't pack "${key}" property of record: ${e}`);
                }
            }
            if (userFieldsDesc !== null) {
    ...
    this.unpack = function(desc, fullyUnpack = false) {
        ...
        case objc.typeVersion:
            ...
            if (nsString === null) { throw new UnpackError(desc, "Can't unpack malformed string descriptor"); }
        ...
        case objc.type128BitFloatingPoint: // caution: lossy precision
            ...
            if (doubleDesc === null) { throw new UnpackError(desc, "Can't coerce 128-bit float to double"); }
        ...
        case objc.typeLongDateTime:
            ...
            if (nsDate === null) { throw new UnpackError(desc, "Can't unpack malformed date descriptor"); }
        ...
        case objc.typeKeyword:
            ...
            return new aesupport.Keyword(name === undefined ? code : name);
        case objc.typeEnum:
        ...
        for (var i = 1; i <= desc('numberOfItems'); i++) {
            ...
            if (key === objc.keyASUserRecordFields) {
                if (valueDesc('descriptorType') !== objc.typeAEList || valueDesc('numberOfItems') % 2 !== 0) {
                    throw new UnpackError(desc, "Malformed AERecord descriptor"); // sanity check
                }
                for (var j = 1; j <= valueDesc('numberOfItems'); j += 2) {
                    const keyString = valueDesc('descriptorAtIndex', j)('stringValue');
                    if (keyString === null) { throw new UnpackError(desc, "Malformed AERecord descriptor"); }
...
function _loadTerminology(appData) { // TO DO: need to merge this into target as it may launch process, in which case we don't want to launch it a second time if 'new instance' flag is set
    ...
    const glueTable = aegluetable.glueForApplication(url);
