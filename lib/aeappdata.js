...
class AEDescriptorBuffer {
	// dynamic-resizing buffer used by AECodecs
	...
	toDescriptor() {
		...
		//console.log('pack FlatDesc:', aebuffer.rawBuffer.length, aebuffer.rawBuffer);
		...
		//console.log('toDescriptor:', `<${(this.rawBuffer.subarray(0, this.offset))}>`);
...
//			appleEvent.setAttributeDescriptor_forKeyword_(considerIgnoreFlags, kae.enumConsidsAndIgnores);
	...
	_writeDescriptor(aeBuffer, value, hasHeader = false) {
		...
		if (value === null) {
			...
			if (value % 1 === 0 && value >= INT32_MIN && value <= INT32_MAX) {
			...
			if (value instanceof Date) {
				...
				const type = cls instanceof aesupport.Keyword ? this.typeCodeForName(cls.name).code : kae.typeAERecord;
				aeBuffer.writeUInt32BE(type);
				const dataSizeOffset = aeBuffer.allocate(4); // alloc UInt32 to hold remaining bytes
				const dataStartOffset = aeBuffer.offset;
				if (hasHeader && type === kae.typeAERecord) { // flattened 'reco' has extra header
					aeBuffer.writeUInt32BE(0); // reserved
					aeBuffer.writeUInt32BE(0); // 4-byte padding
					aeBuffer.writeUInt32BE(0x18); // reserved
				...
				for (let [key, item] of Object.entries(value)) {
					...
					} else if (type === kae.typeAERecord || // if not 'class' property with Keyword value
							   !['class', '#pcls', '0x70636C73', '1885564019'].includes(key)) {
						aeBuffer.writeUInt32BE(this.typeCodeForName(key).code);
						this._writeDescriptor(aeBuffer, item);
						count++;
					}
				}
	...
	pack(value) {
		...
		try {
			...
			throw new Error(`AECodecs.pack couldn't pack ${typeof value}: ${util.inspect(value)}\n${e}`);
		}
	}
	...
	_unpackFourCharCode(rawBuffer) {
		...
		if (type !== kae.typeType && type !== kae.typeEnumerated || size  !== 4) {
			throw new Error(`Malformed specifier: malformed property (not type/enum).`);
		}
	...
	_unpackSpecifierRecord(rawBuffer, expectedEndOffset, fullyUnpack) {
		...
		//console.log(`_unpackSpecifierRecord: reading ${aesupport.formatFourCharCode(rawBuffer.readUInt32BE(descriptorStartOffset))}`);
		const count = rawBuffer.readUInt32BE(rawBuffer.aeoffset);
		if (count !== 4) {
			throw new Error(`Malformed specifier: expected 4 properties, got ${count}`);
		}
		rawBuffer.aeoffset += 8; // step over count and 4-byte padding
		for (var i = 0; i < count; i++) {
			const code = rawBuffer.readUInt32BE(rawBuffer.aeoffset);
			//console.log(`_unpackSpecifierRecord: reading ${aesupport.formatFourCharCode(code)}`);
			...
			case kae.keyAEContainer:
				if (fullyUnpack) {
					...
					const fromDescStartOffset = rawBuffer.aeoffset;
					const size = rawBuffer.readUInt32BE(rawBuffer.aeoffset + 4); // step over type and read size
					const fromDescEndOffset = fromDescStartOffset + 4 + size
					const fromDesc = rawBuffer.subarray(fromDescStartOffset, fromDescEndOffset); // for now, retain original buffer
					rawBuffer.aeoffset = fromDescEndOffset;
					from = new Proxy({specifierRecord: undefined, appData: this, desc: fromDesc}, {
						get: function(object, name) {
							if (object.specifierRecord === undefined) {
								object.specifierRecord = object.appData._readDescriptor(object.desc, true)[aesupport.__specifierRecord];
							}
				...
				break;
			default:
				throw new Error(`Malformed specifier: unknown property.`);
			}
		}
		if (want === undefined || form === undefined || seld === undefined || from === undefined) {
			throw new Error(`Malformed specifier: missing property: {want:${want}, form:${form}, seld:${seld}, from:${util.inspect(from)}}`);
		}
		
		//console.log(`...read: {want:${aesupport.formatFourCharCode(want)}, form:${aesupport.formatFourCharCode(form)}, seld:${seld}, from:${util.inspect(from)}}`);
		...
		case kae.formRange:
			...
			if (seld?.class?.name !== kae.typeRangeDescriptor) { 
				throw new Error(`Malformed specifier: invalid range.`);
			}
		...
		case kae.formTest:
			// this check is arguably redundant if we assume descriptors are always well-formed
			if (!aeselectors.isSpecifier(seld)) {
				throw new Error(`Malformed specifier: invalid test.`); // this doesn't check its-based
			}
			selectors = this.multipleElementsSpecifierAttributes;
			break;
		default:
			throw new Error(`Malformed specifier: unknown form.`);
		}
		if (rawBuffer.aeoffset !== expectedEndOffset) {
			throw new Error(`Malformed record: expected ${expectedEndOffset} bytes, read ${rawBuffer.aeoffset}`);
		}
		return {from,
				want,
				form,
	...
	_unpackAERecord(rawBuffer, expectedEndOffset, fullyUnpack = false, hasHeader = false, rawKeys = false) {
		...
		// note: this not add a 'class' property to returned object
	 // TO DO: move to method so it can be called to presumptively unpack AERecords with custom descriptorType (or - hopefully - throw on anything else)
...
// unpack test clauses
	...
	_readDescriptor(rawBuffer, fullyUnpack = false, hasHeader = false) {
		...
		if (hasHeader) {
			...
			if (header !== 0x646c6532) {
				throw new Error(`_readDescriptor expected flattened AEDesc (#dle2), got: ${aesupport.formatFourCharCode(header)}`);
			}
		...
		//console.log(`_readDescriptor: found '${aesupport.formatFourCharCode(descriptorType)}' size: ${size}`);
		...
		case kae.typeUnicodeText:
			...
			if (bom === (isBE ? 'LE' : 'BE')) { throw new Error(`_readDescriptor: bad BOM in typeUnicodeText`); }
			//if (bom) { rawBuffer.aeoffset += 2; size -= 2; } // TO DO: needed?
			...
			//if (isBE) {
			//console.log(`unpack BE utxt: ${rawBuffer.aeoffset} ${expectedEndOffset}`)
			...
			//} else { // LE
			//console.log(`unpack LE utxt: ${rawBuffer.aeoffset} ${expectedEndOffset}`)
			//	value = rawBuffer.toString('utf16le', rawBuffer.aeoffset, expectedEndOffset);
			//	console.log(rawBuffer.toString('utf16le', rawBuffer.aeoffset, expectedEndOffset))
			//}
			rawBuffer.aeoffset += size;
			break;
		...
		case kae.typeChar: // long-deprecated (but some Carbon apps still use it!)
			value = rawBuffer.toString('latin1', rawBuffer.aeoffset, expectedEndOffset); // not the correct type (which is host machine's primary encoding, e.g. "MacRoman" on English-language Macs), but it will do for now (TBH, simplest solution is to pack, coerce to utf8, and unpack)
			if (size % 2 !== 0) { // align on even byte
				size++;
				expectedEndOffset++;
			}
		...
		case kae.typeFileURL: // temporary
			{
		...
		default:
			try { // is it an AERecord with custom descriptorType? only way to find out is to try to unpack it as 'reco'
				...
				// TO DO: need to unflatten and return as NSAppleEventDescriptor
				throw new Error(`TO DO: can't yet unpack descriptor of type ${aesupport.formatFourCharCode(descriptorType)}`);
				// TO DO: after converting to NSAppleEventDescriptor, check if type is kae.typeAlias [and coerce to typeFileURL?] and unpack as File; also typeFileURL, typeBookmarkData, typeFSRef
			}
		}
		...
		if (expectedEndOffset !== rawBuffer.aeoffset) {
			throw new Error(`_readDescriptor expected '${aesupport.formatFourCharCode(descriptorType)}' descriptor to end on byte ${expectedEndOffset} but ended on ${rawBuffer.aeoffset}`);
		}
		return value;
	}
	...
	unpack(descriptor, fullyUnpack = false) {
		...
		if (err !== 0) {
			throw new Error(`Error ${err}: AECodecs.unpack couldn't flatten malformed descriptor for: ${String(descriptor)}`);
		}
//		console.log('fromDescriptor:', `<${(rawBuffer)}>`);
		rawBuffer.aeoffset = 0;
		try {
			return this._readDescriptor(rawBuffer, fullyUnpack, true);
		} catch (e) {
			throw new Error(`AECodecs.unpack couldn't unpack: ${String(descriptor)}\n\t${e}`);
		}
	}
