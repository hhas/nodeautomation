...
function PackError(value) { // TO DO: take message and/or sub-error as optional args
    ...
    this.value = value;
    this.toString = function() { return `Can't pack value: ${this.value}` }
}

function UnpackError(desc) { // TO DO: take message and/or sub-error as optional args
    this.errorNumber = -1700;
    this.value = desc;
    this.toString = function() { return `Can't unpack descriptor: ${this.value}` }
}
...
function localRunningApplication(url) { // NSURL -> NSRunningApplication/null
    ...
    if (bundle === null) { throw ConnectionError(`Application not found: ${url}`); }
    const bundleID = bundle('bundleIdentifier');
    if (bundleID === null) { throw ConnectionError(`Application not found: ${url}`); }
...
function isRunningWithAddressDescriptor(desc) {
    throw new TypeError('TO DO');
...
function AppData(targetType, targetID, launchOptions) {
    ...
    this._sendAppleEvent = function(appleEvent, sendOptions, timeoutInSeconds) { // used by sendAppleEvent()
        var error = objc.alloc(objc.NSError, objc.NIL);
        var replyEvent = appleEvent('sendEventWithOptions', sendOptions, 
                                    'timeout', timeoutInSeconds, 'error', error.ref()); // throws NSError on AEM errors (but not app errors)
        if (replyEvent === null // 'launch' events normally return 'not handled' errors, so just ignore those
                && error('code') === -1708
                && appleEvent('attributeDescriptorForKeyword', 
                              objc.keyEventClassAttr)('typeCodeValue') === objc.kASAppleScriptSuite
                && appleEvent('attributeDescriptorForKeyword',
                              objc.keyEventIDAttr)('typeCodeValue') === objc.kASLaunchEvent) {
                    return [NSAppleEventDescriptor('recordDescriptor'), null]; // (not a full AppleEvent desc, but reply event's attributes aren't used so is equivalent to a reply event containing neither error nor result)
        }
    ...
    this.sendAppleEvent = function(commandDef, parentSpecifierRecord, parametersObject) { // TO DO: add isRetry = false flag
...
//        try {
            ...
            for (key in parametersObject) {
                // TO DO: trap all errors within loop and rethrow as `Can't pack "${key}" parameter/attribute: ${e}`
                var value = parametersObject[key];
                if (value === undefined) { throw new TypeError(`Bad "${key}" parameter: ${value}`); }
                ...
                if (paramCode !== undefined) {
                    ...
                    case "sendOptions": // all send flags; [array of] keywords, e.g. [k.ignoreReply,...]
                        ...
                        break;
                    case "withTimeout": // TO DO: how to specify default/no timeout? maybe use 0=default, null=none, which decouples our public API from the buggy -[NSAppleEventDescriptor sendEventWithOptions:timeout:]
                        try {
                            timeoutInSeconds = aesupport.SInt32(value);
                        } catch(e) {
                            throw new TypeError(`Bad "timeout" attribute: ${value}`);
                        }
                        break;
            ...
            if (appleEvent('attributeDescriptorForKeyword', objc.keySubjectAttr) === null) {
                // TO DO: throw if subjectAttribute isn't typeNull
...
//        }
    ...
    this.pack = function(value) {
        ...
        if (value.__nodeautomation_pack__ !== undefined) { // value is self-packing, given a targeted AppData instance
            ...
            try {
                ...
                throw new TypeError('TO DO: packing Cocoa objects is not yet supported');
            } catch(e) {
                throw new PackError(value);
            }
        }
    ...
    this.unpack = function(desc, fullyUnpack = false) {
        ...
        case objc.type128BitFloatingPoint:
...
function _loadTerminology(appData) { // TO DO: need to merge this into target as it may launch process
    var url;
    ...
    case "named":
        ...
        break;
    case "at": // eppc: URL
        throw new TypeError('TO DO: eppc not yet supported.');
    case "ID":
