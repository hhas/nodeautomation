...
// TO DO: change `__nodeautomation_keywordName__` and other internal-use property names to Symbol-based keys, e.g. `const __keywordName = Symbol('__keywordName')`; eliminating any possibility of name conflicts and the awkward pseudo-namespacing that string-based names require
...
function readRelaunchMode(value) {
    ...
    const flag = kRelaunchModes[value.__nodeautomation_keywordName__];
    if (flag === undefined) {
...
const kMissingValueDescriptor = objc.NSAppleEventDescriptor.descriptorWithTypeCode_(objc.ns(kae.cMissingValue));
...
function fileIDForURL(url) { // NSURL -> NSData // used in determining if two existing file URLs identify same inode
    const fileIDRef = new objc.InOutRef();
...
function launchApplicationAtURL(url) { // NSURL -- fileURL
    const config = objc.NSDictionary.dictionaryWithObject_forKey_(kLaunchEvent, 
    															  objc.ns('NSWorkspaceLaunchConfigurationAppleEvent'));
    var error = new objc.InOutRef();
...
function AppData(targetType, targetID, options) {
    ...
    this.target = function() {
        if (!this._targetDescriptor) {
            ...
            case "at": // eppc: URL
                ...
                var url = objc.NSURL.URLWithString_(objc.ns(this.targetID));
    ...
    this.isRunning = function() {
        ...
        case 'at': // "eppc" URL
            var url = objc.NSURL.URLWithString_(objc.ns(url));
    ...
    this.launch = function() { // called by Application.launch()
        ...
        if (this.isRunning()) {
            ...
            case 'at':
                ...
                //launchApplicationAtURL(objc.NSURL.URLWithString_(objc.ns(this.targetID)));
                throw new Error('TBC');
                return;
            case 'ID':
                if (typeof this.targetID !== 'string') {
                ...
                var url = NSWorkspace.sharedWorkspace().urlForApplicationWithBundleIdentifier_(objc.ns(this.targetID));
                if (!url) {
...
//        }
    ...
    this.pack = function(value) {
        ...
        case "string":
            return objc.NSAppleEventDescriptor.descriptorWithString_(objc.ns(value));
        }
        if (value.__nodeautomation_pack__ !== undefined) { // value is self-packing, given a targeted AppData instance
            ...
            return objc.NSAppleEventDescriptor.descriptorWithDate_(objc.ns(value));
            ...
            if (customType !== undefined && aesupport.Keyword.isKeyword(customType)) { // 'class' property contains a type name?
            ...
            for (var key in value) {
                try {
                    if (isCustomRecordType && (key === 'class' || kae.pClass == key)) { continue; }
                    ...
                    if (keyCode === undefined) {
                        if (key.startsWith('$')) {
                            ...
                            const keyDesc = objc.NSAppleEventDescriptor.descriptorWithString_(objc.ns(key.substr(1)));
