...
const kLimitedRelaunchEvents = [objc.kCoreEventClass + '/' + objc.kAEOpenApplication,  // `someApp.run()`
                                objc.kASAppleScriptSuite + '/' + objc.kASLaunchEvent]; // `someApp.launch()`
...
function unpackSpecifierRecord(appData, desc, fullyUnpack) {
    ...
    case objc.typeObjectSpecifier:
        ...
        case objc.formRelativePosition:
            ...
            break;
        case objc.formRange:
            const start = appData.unpack(seld('descriptorForKeyword', objc.keyAERangeStart), true);
            const stop  = appData.unpack(seld('descriptorForKeyword', objc.keyAERangeStop), true);
