...
const kDefaultRelaunchOption = kRelaunchOptions.limitedRelaunch;

function readRelaunchOption(value) {
    if (!aesupport.isKeyword(value)) {
         throw new TypeError(`Bad "relaunchOption" value (not a keyword): ${aeformatter.formatValue(value)}`);
    }
    const flag = kLaunchOptions[value.__nodeautomation_keywordName__];
    if (flag === undefined) {
        throw new TypeError(`Bad "relaunchOption" value (unknown "${value}" keyword): ${value}`);
    }
    return flag;
}
//        } else if (kRelaunchOptions[item] !== undefined) {
//            relaunch = kRelaunchOptions[item];
...
function AppData(targetType, targetID, options) {
    ...
    this.relaunchMode = (options.autoRelaunch !== undefined ? readRelaunchOption(options.autoRelaunch)
                                                            : kDefaultRelaunchOption);
