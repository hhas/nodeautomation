...
function processDescriptorForLocalApplication(url, launchOptions) { // NSURL, NSWorkspaceLaunchOptions
    ...
    if (!runningProcess) {
        ...
        runningProcess = objc.NSWorkspace('sharedWorkspace')('launchApplicationAtURL', url, 
                                                             ...
                                                             'error', error);
        if (!runningProcess) { throw new TypeError(`Can't launch ${url}: ${error.deref()}`); }
    }
...
function launchApplicationAtURL(url) {
    ...
    const runningProcess = objc.NSWorkspace('sharedWorkspace')('launchApplicationAtURL', url, 
                                                               ...
                                                               'error', error);
    if (!runningProcess) { throw new aeerrors.ConnectionError(error.deref()('code'), url); }
}
            ...
            case "ID":
                ...
                case 'string': // bundleIdentifier
                    ...
                    var runningProcess = objc.NSWorkspace('sharedWorkspace')(
                                                          ...
                                                          'error', error);
                    if (!runningProcess) {
                        throw new TypeError(`Can't launch application "${this.targetID}": ${error.deref()}`);
                    }
