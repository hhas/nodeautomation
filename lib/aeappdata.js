...
class AppData {
	...
	constructor(targetType, targetID, options) {
		...
		if (options.logAppleEvents) { // TO DO: and/or take a function
			...
			this._sendAppleEvent = function (aeBuffer, sendOptions, timeout) {
				...
				const [reply, error] = send(aeBuffer, sendOptions, timeout);
	...
	target() {
		...
		// TO DO: this needs to return Buffer
		...
		if (!this._targetDescriptor) {
			...
			case "ID":
				...
				case 'function': // NSAppleEventDescriptor?
					if (aesupport.isNSAppleEventDescriptor(this.targetID)) { return this.targetID; } // caution: it is user's responsibility to ensure supplied descriptor is a valid AEAddressDesc
			...
			// TO DO: kludge; this gives us an AEOpaqueDescriptor, albeit via roundabout route
			this._targetDescriptor = this.unpack(desc);
		}
	...
	_sendAppleEvent(aeBuffer, sendOptions, timeout) { // used by sendAppleEvent()
		...
		const event = aeBuffer.toNSAppleEventDescriptor();
		const error = new objc.Ref();
		const replyEvent = event.sendEventWithOptions_timeout_error_(sendOptions, timeout, error); // TO DO: replace with AESendMessage
		return [replyEvent, replyEvent ? null : error.deref().code()]; // TO DO: is there reliable way to check if NSError** is nilptr?
	}
		...
		const event = objc.NSAppleEventDescriptor
				.appleEventWithEventClass_eventID_targetDescriptor_returnID_transactionID_(
																...
																kae.kASLaunchEvent,
															  	...
															  	kae.kAutoGenerateReturnID,
															  	kae.kAnyTransactionID);
		const [replyEvent, errorCode] = this._sendAppleEvent(event, kSendOptions.waitReply, 30);
		if (!replyEvent) { return errorCode; }
	...
	_readParametersObject(commandDef, parametersObject, parentSpecifierRecord) {
		...
		if (parentSpecifierRecord.form !== aesupport.kSpecifierRoot) {
			const parentSpecifier = {[aesupport.__packSelf]: function(aeBuffer, appData) {
	...
	_packAppleEvent(commandDef, addressDesc, attributes, parameters, sendOptions, timeout) {
		...
		event.writeUInt32BE(0);
		const parametersJumpOffset = event.allocate(4); // offset to parameters
		event.writeUInt32BE(4);
		const parametersJumpStart = event.offset;
		...
		addressDesc[aesupport.__packSelf](event, this);
		...
		event.rawBuffer.writeUInt32BE(parametersJumpEnd - parametersJumpStart, parametersJumpOffset);
	...
	sendAppleEvent(commandDef, parentSpecifierRecord, parametersObject) {
		let replyEvent = null, errorCode = null;
		try {
			...
			const aeBuffer = this._packAppleEvent(commandDef, addressDesc, 
												  attributes, parameters, 
												  sendOptions, timeout);
			
			// send the AppleEvent
			[replyEvent, errorCode] = this._sendAppleEvent(aeBuffer, sendOptions, timeout);			
			...
			if (!replyEvent) {
				if (kRelaunchableErrorCodes.includes(errorCode) && this.isRelaunchable(commandDef)) {
					...
					if (newAddressDesc.size === aeBuffer.addressDescEndOffset - aeBuffer.addressDescStartOffset) {
						const endOffset = aeBuffer.offset;
						aeBuffer.offset = aeBuffer.addressDescStartOffset;
						newAddressDesc[aesupport.__packSelf](eventBuffer, this);
						aeBuffer.offset = endOffset;
					}
					[replyEvent, errorCode] = this._sendAppleEvent(aeBuffer, sendOptions, timeout);
				}
				if (!replyEvent) { throw new aeerrors.AppleEventManagerError(errorCode); }
			}
			...
			*/
			...
			if (value instanceof Date) {
				...
				if (value.isKindOfClass_(objc.NSAppleEventDescriptor)) {
					const aedesc = value.aeDesc().value; // borrow the AEDesc struct
					const rawBuffer = flattenAEDesc(aedesc.ref()); // and pass a pointer to it to AEFlattenDesc
					const startOffset = aeBuffer.allocate(size - 8);
...
function _loadTerminology(appData) { // TO DO: need to merge this into target as it may launch process, in which case we don't want to launch it a second time if 'new instance' flag is set
	...
	if (appData.terminologyTables === null) {
		...
		case "ID":
			...
			case 'function': // NSAppleEventDescriptor?
				if (aesupport.isNSAppleEventDescriptor(appData.targetID)) { // 
